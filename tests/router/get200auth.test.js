...
test('GET authenticated routes', (t) => {
  ...
  dbReset().then(() => {
    ...
    request(app).post('/login').type('form').send({email: 'jam@gmail.com', password: 'password'}).end((getCookieErr, loginRes) => {
      const cookies = loginRes.headers['set-cookie'];
      authenticatedPages.forEach((page) => {
        // Then make requests to each authenticated route
        // setting cookie for each request
        request(app).get(`/${page}`).set('Cookie', cookies).expect(200).end((err, res) => {
